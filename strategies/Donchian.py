...
class Donchian(strategies.BaseStrategy.BaseStrategy):
    ...
    def __init__(self):
        super().__init__()
        for d in self.datas:
            self.add_indicator(d,'dc20',indicators.DonchianChannel,period=80)
    ...
    def next(self):
        ...
        for i,d in enumerate(self.datas):
            ...
            if self.get_indicator(d, 'dc20').exitlong[0] and self.getposition(d).size > 0:
                self.close(data=d)
            elif self.get_indicator(d, 'dc20').exitshort[0] and self.getposition(d).size < 0:
                self.close(data=d)
